...
local function aggregateModifierReductions(effects)
    ...
    for _, effect in pairs(effects or {}) do
        ...
        for school, amount in pairs(effect.resistanceReduction or {}) do
            groups[group][school] = math.max(groups[group][school] or 0, tonumber(amount) or 0)
        end
    end
...
function G:ActiveTargetModifiers(encounter, targetResistance)
    ...
    for knowledgeName, semantics in pairs(XelAssistKnowledge or {}) do
        if (semantics.armorDebuff or semantics.resistanceDebuff) and not byName[knowledgeName] then
    ...
    return aggregateModifierReductions(effects),
        aggregateDamageTakenModifiers(effects),
