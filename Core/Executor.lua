...
local function targetGuid(plan, unit)
    if plan.targetRef and plan.targetRef.guid ~= nil then return plan.targetRef.guid end
...
local function validateFriendly(owner, plan)
...
local function dispatchPlayer(action, plan, castName, friendly, capturedGuid)
    if action.facts.ground then CastSpellByName(castName, "CLICK")
    elseif friendly then CastSpellByName(castName, capturedGuid)
    elseif plan.target == "target" and QueueSpellByName then QueueSpellByName(castName)
    elseif plan.target then CastSpellByName(castName, plan.target)
...
function XA:ExecutePlayerPlan(plan, selected)
    ...
    local relation = plan.targetRef and plan.targetRef.relation or plan.targetRelation
    local friendly = friendlyRelation(relation)
    local unit = plan.target or ((not facts.ground) and "target" or nil)
    local capturedGuid, reason
    local forceQueue = not facts.ground and not friendly
    ...
    if friendly then
        unit, capturedGuid, reason = validateFriendly(self, plan)
        if not unit then self:Fallback(reason); XelAssist.UI.HUD:Refresh(true); return end
    end
    if XelAssist.Game.Capabilities:InRange(castName, unit) == false then
        self.lastReason = "Move into range — " .. action.name
        XelAssist.UI.HUD:Refresh(true); return
    end
    if friendly and not XelAssist.Game.Capabilities:SameUnitRef(plan.targetRef) then
        self:Fallback("ally changed"); XelAssist.UI.HUD:Refresh(true); return
    end
    local playerGuid = self:PlayerGUID()
    local applicationGuid = friendly and capturedGuid or targetGuid(plan, unit)
    local duplicate = duplicateApplication(self, action, plan.tooltip, unit,
        applicationGuid, playerGuid)
    if duplicate then self:Fallback(duplicate); return end
    ...
    dispatchPlayer(action, plan, castName, friendly, capturedGuid)
    self:RecordDecision(plan, selected)
    if XelAssist.Combat.Observations then
        XelAssist.Combat.Observations:Submitted(action,
            friendly and capturedGuid or plan.target, plan.tooltip)
    end
    if applicationGuarded(facts, plan.tooltip) then
        self:MarkAuraPending(action.name,
            math.max(2, (plan.wait or 0) + (plan.cast or 0) + 2),
            applicationGuid, action.spellId, playerGuid, auraBarForFacts(facts))
    end
    self.lastReason = action.name .. " — " .. plan.reason
    XelAssist.UI.HUD:Refresh(true)
end
