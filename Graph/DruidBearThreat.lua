-- Branch-local Bear/Dire Bear all-school threat multiplier.  Its value
-- emerges only through later player threat; form edges receive no flat score.
XelAssist.Graph.DruidBearThreat = {}
local B = XelAssist.Graph.DruidBearThreat
...
local function runtime()
    return XelAssist.Game and XelAssist.Game.Player
        and XelAssist.Game.Player.DruidBearThreat
end
...
local function exactProfile(value)
    ...
    return owner and type(value) == "table" and value.valid == true
        ...
        and value.bearForm == owner.BEAR_FORM
        and value.direBearForm == owner.DIRE_BEAR_FORM
...
local function sync(state, component)
    ...
    if not (owner and profile and current ~= nil) then
        if component then
            ...
            component.reason = "Druid Bear threat form evidence unavailable"
        end
        return false
    end
    local active = owner:IsBearForm(current)
...
function B:Attach(state)
    ...
    local out = { available = false, exact = false,
        ...
        source = "exact Druid form and Bear threat passive profile" }
    state.druidBearThreat = out
    if not exactProfile(profile) then
        out.reason = profile and profile.reason
            or "Druid Bear threat profile unavailable"
        return false
    end
...
function B:Copy(source, target)
    if not (source and target and source.druidBearThreat) then return false end
    local component = source.druidBearThreat
    target.druidBearThreat = copy(component)
    return sync(target, target.druidBearThreat)
end

-- DruidForms applies and synchronizes the exact form transition first.
function B:AfterForm(state)
    local component = state and state.druidBearThreat
...
-- Compose after any other player-owned threat factors. Pets and other actors
-- deliberately bypass this form-owned multiplier.
function B:Resolve(state, actor, baseMultiplier, baseExact)
    ...
    if not (component.available == true and component.exact == true
        ...
        and multiplier == (runtime():IsBearForm(current) and 1.30 or 1)) then
