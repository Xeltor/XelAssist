...
local function consumerKey(context)
    ...
    if value == nil and preparation then
        value = preparation:ConsumerKey(
            context.state, context.action, context.tooltip)
...
function C:Build(context)
    ...
    local out = {
        ...
        dotRawDirectPower = context.dotRawDirectPower, dotRawPeriodicPower = context.dotRawPeriodicPower,
        dotPeriodicExpectedPower = context.dotPeriodicExpectedPower, periodicUndeliveredPower = context.periodicUndeliveredPower,
        periodicRefresh = context.periodicRefresh,
